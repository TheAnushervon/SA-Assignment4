...
from filters import (
    BnWFilter, BlurFilter, MirrorFilter,
    ResizeFilter, ShowFilter, EdgeDetectionFilter
)
...
def top_down_approach():
    ...
    resize_filter = ResizeFilter(
        scale_factor=0.5, outputs=[mirror_filter.input])

    input_show_filter = ShowFilter(
        window_name='Input video',
        outputs=[resize_filter.input]
    )
...
def pipe_approach():
    pipe = ShowFilter('Input Video').pipe(EdgeDetectionFilter()).pipe(
        ResizeFilter(0.5)).pipe(ShowFilter(outputs=[sink_pipe.put]))
